---
layout: home
title: 'Bienvenue'
---

# Salut, je suis Karl Sondeji ! 👋🏾

Aspirant Data Enginner passionné par l'intelligence artificielle, l'automatisation de pipelines et le développement d'outils pour résoudre des problèmes concrets.

## 🎯 Mon Expertise

- Mise en place de pipelines de données évolutifs et efficaces
- Visualisation de données interactive
- Déploiement d'applications et d'outils automatisés en production

[Découvrez mon parcours](/about/)

## 📊 Projets à la Une

Voici une sélection de projets sur lesquels j'ai travaillé :

- [Sécurisation d'une API vulnérable contre les cyberattaques](https://github.com/karl732/Securisation-API-vulnerable)
- [Pipeline ETL Orchestrée avec rapport Power BI sur Microsoft Fabrics](https://github.com/karl732/fabric-wind-power-analytics)

[Voir tous mes projets](/projets/)
